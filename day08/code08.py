...
def createAntinodes(m: list[list[str]], antennas: Dict[str, List[Tuple[int, int]]]):
    maxI = len(m) - 1
    maxJ = len(m[0])-1
    
    
    for ant in antennas.keys():
        # print('looking at ', ant, ' ', antennas[ant])
...
def createAntinodesPart2(m: list[list[str]], antennas: Dict[str, List[Tuple[int, int]]]):
    maxI = len(m) - 1
    maxJ = len(m[0])-1
...
def countAntiNodes(m: list[list[str]]):
    sum = 0
    for row in m:
        for tile in row:
            if tile == '#':
